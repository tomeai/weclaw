    ...
    for ag_ui_msg in ag_ui_messages:
        ...
        if isinstance(ag_ui_msg, (DeveloperMessage, SystemMessage)):
            ...
            user_msg = Message(
                ...
                type=MessageType.MESSAGE,
                role=Role.USER,
                content=(user_content if user_content else [TextContent(text='')]),
            )
            converted_messages.append(user_msg)
...
class AGUIAdapterUtils:
    ...
    def convert_agui_request_to_agent_request(
        ...
        if agui_request.tools:
            tools = [self.convert_ag_ui_tool(tool).model_dump() for tool in agui_request.tools]
        else:
            tools = []

        agent_request = AgentRequest.model_validate(
            {
                'input': [msg.model_dump(exclude_none=True) for msg in converted_messages],
    ...
    def convert_ag_ui_tool(self, ag_tool: AGUITool) -> Tool:
        ...
        if schema_type == 'object' and isinstance(properties, dict):
            if required is not None and not (isinstance(required, list) and all(isinstance(x, str) for x in required)):
                required = None
            ...
            converted_params: FunctionParameters | Dict[str, Any] = fp
        else:
    ...
    def _convert_message_event(
        ...
        if message_event.status in {RunStatus.Completed}:
            agui_message_ids = self._message_id_to_agui_message_id_mapping[message_event.id]
            for agui_message_id in agui_message_ids:
                ...
                if not agui_message_status:
                    logger.warning(
                        'AG UI message not started before Agent API message completed: %s',
                        agui_message_id,
                    )
                    continue
                ...
                if agui_message_status != AGUI_MESSAGE_STATUS.COMPLETED:
                    ...
                    self._agui_message_status[agui_message_id] = AGUI_MESSAGE_STATUS.COMPLETED
    ...
    def _get_msg_content_index(self, content: Content) -> int:
        values = sorted(
            [str(v) for k, v in vars(ContentType).items() if not k.startswith('_') and isinstance(v, str)],
        )
    ...
    def _convert_content_event(self, content: Content) -> List[AGUIEvent]:
        ...
        def _ensure_agui_text_message_started(
            ...
            if agui_msg_id in self._agui_message_status:
                return []
            self._agui_message_status[agui_msg_id] = AGUI_MESSAGE_STATUS.CREATED
            return [
                TextMessageStartEvent(
        ...
        if isinstance(content, TextContent):
            ...
            if content.delta:
                if self._agui_message_status[agui_msg_id] == AGUI_MESSAGE_STATUS.COMPLETED:
                    logger.warning(
                        'Message already completed: %s',
                        agui_msg_id,
                    )
                elif content.text:
                    self._agui_message_status[agui_msg_id] = AGUI_MESSAGE_STATUS.IN_PROGRESS
                    events.append(
                        TextMessageContentEvent(
                            message_id=agui_msg_id,
                            delta=content.text,
                ...
                if self._agui_message_status[agui_msg_id] == AGUI_MESSAGE_STATUS.IN_PROGRESS:
                    events.append(
                        TextMessageEndEvent(
                            message_id=agui_msg_id,
                        ),
                    ...
                    self._agui_message_status[agui_msg_id] = AGUI_MESSAGE_STATUS.COMPLETED
                elif self._agui_message_status[agui_msg_id] == AGUI_MESSAGE_STATUS.CREATED:
                    events.append(
                        TextMessageContentEvent(
                            message_id=agui_msg_id,
                            delta=content.text,
                    ...
                    logger.warning(
                        'AG UI message stream is completed for the text content: %s',
                        content.text,
                    )
            ...
            if agui_msg_id in self._agui_message_status:
                logger.warning(
                    'AG UI message stream is completed for the tool call content: %s',
                    content.data,
                )
            elif content.status == RunStatus.Completed:
                X = TypeAdapter(FunctionCall | FunctionCallOutput)
                ta = TypeAdapter(X)

...
export default function ProfileLayout({ children, sidebar }: ProfileLayoutProps) {
  ...
  useEffect(() => {
    ...
    console.log('[ProfileLayout] 认证状态检查:', {
      user: !!user,
      isJwtAuthenticated,
      pathname,
      isInitialized
    ...
    if (!isLoggedIn) {
      ...
      console.log('[ProfileLayout] 用户未登录，重定向到登录页面')
      router.push(`/auth/login?callbackUrl=${encodeURIComponent(pathname)}`)
    } else {
      console.log('[ProfileLayout] 用户已登录，停止检查状态')
      setIsChecking(false)
    }
  ...
  if (!isInitialized || isChecking || (!user && !isJwtAuthenticated)) {
    console.log('[ProfileLayout] 显示加载状态:', { isInitialized, isChecking, user: !!user, isJwtAuthenticated })
    return (
